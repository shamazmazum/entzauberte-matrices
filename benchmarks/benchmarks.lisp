...
;; Some of my functions are marked as pure or may be marked as so in
;; the future. This forces evaluation.
(defun evaluate (x) x)
...
(defun vstack (n times)
  ...
  (let ((m (make-matrix n n)))
    (tb:with-timing (times)
      (evaluate (em:vstack (list m m m) 'double-float))))
...
(defun hstack (n times)
  ...
  (let ((m (make-matrix n n)))
    (tb:with-timing (times)
      (evaluate (em:hstack (list m m m) 'double-float))))
